...
const config = {
  // TODO: to ask mobile dev, how to use this in package
...
const ViewNotificationItem = ({
  ...
  const { originalBody: parsedBody, timeStamp } = extractTimeStamp(
    notificationBody || ''
  );
  ...
  const ctaStyles = {
    ...
    borderWidth: 1,
    borderRadius: 0, // TODO(M): ts error
  };
              ...
              {!timeStamp ? null : (
                <View style={styles.timestampOuter}>
                  <Text style={styles.timestamp}>
                    {/* TODO(M): check and try with date-fns */}
                    {moment
